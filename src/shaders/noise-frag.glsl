...
vec3 noise(vec3 pos)
{
	...
	vec3 f = fract(pos);
	//f = f * (5.0-2.0*f);
    f = vec3(u_Color) * vec3((sin(f.x * 2.0f ) + 1.0f), (sin(f.y * 2.0f ) + 1.0f), (sin(f.z * 2.0f ) + 1.0f));
...
void main()
{
    ...
    vec2 worley = worley(vec3(fs_Pos.xyz) * 3.5, 0.6);
    vec3 baseColor = (vec3(fs_Pos.xyz) + u_Color.rgb) * 0.8f;
    out_Col = vec4(fbm(vec3(worley * fluc, fluc)+ baseColor + u_Time * 0.003) * 0.5f, 0.7f);
}